...
async function populate() {
    try {
        ...
        const wallpapers = await processWallpaperss(mockWallpapers);
...
async function processWallpaperss(wallpapers) {
...
async function findWallpaperByTitle(title) {
    const wallpaper = await database
...
async function processCollectionsRecords(collectionsRecords) {
    ...
    for (let collectionRecord of collectionsRecordsList) {
        const collection = await findCollectionByTitle(
            collectionRecord.collection
        );
        collectionRecord.collectionId = collection._id;
        delete collectionRecord.collection;

        const wallpaper = await findWallpaperByTitle(
            collectionRecord.wallpaper
        );
